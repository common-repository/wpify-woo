...
class FreeShippingNoticeModule extends AbstractModule {
				...
				<div>
					<?php if ( ! empty( $this->get_setting( 'show_icon' ) ) ) { ?>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 61.03"
							...
							<path
								style="<?php echo $this->get_setting( 'icon_color' ) ? sprintf( 'fill: %s;', $this->get_setting( 'icon_color' ) ) : ''; ?>"
	...
	public function get_percentage_for_free_shipping() {
		$percentage = 0;
		if ( $this->get_cart_amount() && $this->get_free_shipping_amount() ) {
			$percentage = round( ( ( $this->get_cart_amount() ) / $this->get_free_shipping_amount() ) * 100 );
		}
	...
	public function get_free_shipping_amount() {
		...
		$amount = floatval( apply_filters( 'woocs_exchange_value', $amount ) );
