...
class PacketaShippingModule extends AbstractModule {
	...
	public function setup() {
		...
		if ( ! defined( 'DISABLE_NAG_NOTICES' ) || ! DISABLE_NAG_NOTICES ) {
			...
			add_action( 'admin_notices', [ $this, 'packeta_deprecated_notice' ] );
		}
		add_action( 'wp_ajax_wpify_packeta_dismiss_notice', [ $this, 'wpify_packeta_dismiss_admin_notice' ] );
		add_action( 'wp_ajax_wpify_packeta_removed_dismiss_notice', [
			$this,
			'wpify_packeta_removed_dismiss_admin_notice'
		] );
	...
	public function packeta_order_column_content( $column ) {
		...
		if ( 'packeta' === $column ) {
			...
			if ( $order->is_packeta_shipping() && ! $packetaId ) {
				...
				?>
				<a id="packeta-details-link" class="button packeta-button packeta-button--edit"
				   href="javascript:void(null);" data-order_id="<?php echo $post->ID; ?>"
				...
				<a class="button packeta-button packeta-button--send"
				   href="<?php echo add_query_arg( array(
					   'action'   => 'send_to_packeta',
					   'order_id' => $order->get_id(),
					   'redirect' => urlencode( esc_url( $url ) )
				   ), admin_url() ); ?>"
	 ...
	 * Notice that the Packeta module will be removed
	 */
	function packeta_deprecated_notice() {
		$string     = __( 'The Packeta module will be removed from the WPify Woo plugin in following master version. Please install the official plugin early to avoid loosing the Packeta shipping method. This warning disappears when the module is deactivated.', 'wpify-woo' );
		$button     = __( 'Install official Packeta plugin', 'wpify-woo' );
		$plugin_url = wp_nonce_url(
			add_query_arg(
				array(
		...
		printf( '<div class="notice notice-error wpify-packeta-notice"><p>%s</p><p><a href="%s" class="button action">%s</a></p></div>', $string, $plugin_url, $button );
	}
}
